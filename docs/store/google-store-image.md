## Google Play 스크린샷 가이드

### 스크린샷 개요
- 짧은 제목 + 바로 이해되는 기능 문구로 구성
- 1장: 홈 화면과 빠른 시작
- 2장: 풀스크린 드로잉 캔버스
- 3장: 브러시와 색상 팔레트
- 4장: 사진 위 드로잉
- 5장: 작품 보관함과 저장
- 6장: 설정 / 후원형 Premium

---

## 스크린샷 구성 (6장)

### 1장: 홈 화면과 빠른 시작
- **제목(KO)**: **간단한 그림판**
- **부제목(KO)**: **열면 바로 그리기**
- **Title(EN)**: **Simple Drawing Pad**
- **Subtitle(EN)**: **Open and draw**

> 1장은 첫인상이므로 기능 나열 대신 **"이건 간단한 그림판이다"** 한 가지만 전달한다.
> 드로잉·캔버스·툴 같은 앱 용어보다 그림판·낙서·그리기처럼 누구나 아는 말을 쓴다.

**캡처 화면**: 홈 화면에서 최근 작품 히어로와 `Start Drawing` / `My Artworks` 진입이 보이는 상태
- **포함 요소**:
  - 최근 작품 3장을 겹친 종이 묶음 히어로
  - 홈 주요 CTA(`Start Drawing`)와 작품 보관함 진입(`My Artworks`)
  - 설정 / 프리미엄 진입 행
  - 무료 버전이면 하단 배너 광고 영역
  - 앱이 단순하다는 인상을 주도록 요소 최소화

---

### 2장: 풀스크린 드로잉 캔버스
- **제목(KO)**: **넓은 캔버스**
- **부제목(KO)**: **방해 없이 그리기**
- **Title(EN)**: **Full Canvas**
- **Subtitle(EN)**: **Draw with focus**

**캡처 화면**: 실제 선이 그려진 드로잉 화면
- **포함 요소**:
  - 넓은 종이색 캔버스에 실제 그림이 그려져 있는 상태
  - 상단 툴바의 저장 / 공유 / 실행 취소 / 다시 실행 / 내 작품 / 더보기
  - 하단 3단 툴바(브러시 퀵 · 굵기 슬라이더 · 색상 퀵)
  - 광고나 다이얼로그가 화면을 가리지 않는 상태

---

### 3장: 브러시와 색상 팔레트
- **제목(KO)**: **10종 브러시**
- **부제목(KO)**: **펜·연필·마커·수채화·에어브러시 외**
- **Title(EN)**: **10 Brushes**
- **Subtitle(EN)**: **Pen, pencil, marker, and more**

**캡처 화면**: 브러시 선택 시트 또는 색상 선택 시트가 열린 화면
- **포함 요소**:
  - 10종 브러시 셀렉터(펜, 연필, 마커, 붓, 형광펜, 만년필, 크레파스, 수채화, 에어브러시, 지우개)
  - 16색 팔레트 + 커스텀 컬러 피커(HEX 입력)
  - 브러시 크기 슬라이더와 굵기 미리보기
  - 수채화 / 에어브러시의 잠금 표시(보상형 광고 또는 Premium으로 해금)

---

### 4장: 사진 위 드로잉
- **제목(KO)**: **사진 위에 그리기**
- **부제목(KO)**: **따라 그리고 메모하기**
- **Title(EN)**: **Draw on Photos**
- **Subtitle(EN)**: **Trace and mark up**

**캡처 화면**: 갤러리 사진을 참조로 깔고 그 위에 선을 그린 상태
- **포함 요소**:
  - 배경에 깔린 사진과 그 위의 실제 드로잉
  - 사진 불러오기 진입(더보기 시트)
  - 참조 사진이 작품과 함께 보존된다는 맥락
  - 사진과 선이 구분되도록 대비가 좋은 예시 이미지 사용

---

### 5장: 작품 보관함과 저장
- **제목(KO)**: **저장하고 이어 그리기**
- **부제목(KO)**: **보관함에서 다시 열기**
- **Title(EN)**: **Save and Reuse**
- **Subtitle(EN)**: **Reopen anytime**

**캡처 화면**: 앱 내 작품 보관함 그리드 또는 저장 옵션 시트
- **포함 요소**:
  - 저장된 작품 썸네일 2열 그리드와 작품 수 표시
  - 작품 열기(탭) / 삭제(길게 누름) 동작
  - 저장 시트의 PNG·JPEG 포맷과 1x / 2x / 3x 해상도 선택
  - 기기 갤러리 저장과 Android 공유로 이어지는 흐름

---

### 6장: 설정 / 후원형 Premium
- **제목(KO)**: **광고 없이 그리기**
- **부제목(KO)**: **한 번 후원으로 깔끔하게**
- **Title(EN)**: **Ad-Free Drawing**
- **Subtitle(EN)**: **One-time support**

**캡처 화면**: Premium 화면 또는 설정 화면
- **포함 요소**:
  - Small / Medium / Large 3개 후원 옵션과 Medium 인기 배지
  - 광고 제거 / Premium 브러시 접근 혜택 영역
  - 구매 복원 버튼과 하단 고정 구매 CTA
  - 설정 화면이면 언어, 진동, 브러시 가이드, 지우기 전 확인, 흔들어 지우기, 저장 해상도/포맷

---

## 제목·부제목 추가 후보군

> 규칙: 제목·부제목 모두 **2~3단어**를 넘지 않는다. (위 6장 구성안은 "기본안"으로 유지)

### 후보군 A — 행동 중심
바로 무엇을 할 수 있는지 동사로 전달. 첫 인상에서 "쓰기 쉬움"을 강조할 때.

| 장 | 제목(KO) | 부제목(KO) | Title(EN) | Subtitle(EN) |
| --- | --- | --- | --- | --- |
| 1 | 쉬운 그림판 | 켜고 바로 그리기 | Easy Drawing Pad | Tap and draw |
| 2 | 화면 가득 | 방해 없는 화면 | Edge to Edge | Distraction free |
| 3 | 브러시 10종 | 질감까지 다르게 | Ten Brushes | Real textures |
| 4 | 사진 위에 | 따라 그리기 | On Your Photos | Trace it |
| 5 | 언제든 이어서 | 열고 계속 그리기 | Pick Up Later | Reopen anytime |
| 6 | 광고 없이 | 한 번 후원 | No Ads | One-time support |

### 후보군 B — 감성 / 톤 중심
앱의 "종이 + 잉크" 테마를 그대로 문구로 옮긴 안. 디자인 톤과 가장 잘 붙는다.

| 장 | 제목(KO) | 부제목(KO) | Title(EN) | Subtitle(EN) |
| --- | --- | --- | --- | --- |
| 1 | 가벼운 그림판 | 새 종이처럼 | A Light Sketchpad | Like fresh paper |
| 2 | 종이처럼 넓게 | 여백 가득 | Paper Wide Open | Room to sketch |
| 3 | 펜부터 수채화까지 | 브러시 10종 | Pen to Watercolor | 10 brushes |
| 4 | 사진도 종이로 | 위에 덧그리기 | Photos as Paper | Draw over it |
| 5 | 그림은 남는다 | 보관함에 저장 | Nothing Gets Lost | Saved locally |
| 6 | 깔끔한 캔버스 | 광고 없는 후원 | Clean Canvas | Ad-free forever |

### 후보군 C — 혜택 / 키워드 중심 (ASO)
검색 키워드를 문구에 직접 노출. 스토어 검색 유입을 우선할 때.

| 장 | 제목(KO) | 부제목(KO) | Title(EN) | Subtitle(EN) |
| --- | --- | --- | --- | --- |
| 1 | 간단 그림판 | 무료 낙서 앱 | Simple Paint App | Free doodle app |
| 2 | 풀스크린 드로잉 | 넓은 작업 공간 | Fullscreen Drawing | More space |
| 3 | 브러시 & 색상 | 16색 + 커스텀 | Brushes & Colors | 16 colors, custom |
| 4 | 사진 위 드로잉 | 밑그림 따라 그리기 | Draw on Photo | Trace a picture |
| 5 | 저장 & 공유 | PNG · JPEG 저장 | Save & Share | PNG or JPEG |
| 6 | 프리미엄 후원 | 광고 제거 해금 | Premium Support | Ads removed |

### 1장 전용 추가 후보 (간단한 그림판 강조)
1장은 설치를 결정짓는 화면이라 후보를 넓게 둔다. 모두 **쉬운 단어 + 2~3단어** 원칙을 지킨다.

| 번호 | 제목(KO) | 부제목(KO) | Title(EN) | Subtitle(EN) | 뉘앙스 |
| --- | --- | --- | --- | --- | --- |
| 1 | 간단한 그림판 | 열면 바로 그리기 | Simple Drawing Pad | Open and draw | 기본안, 가장 무난 |
| 2 | 쉬운 그림판 | 켜고 바로 그리기 | Easy Drawing Pad | Tap and draw | 행동 유도 |
| 3 | 가벼운 그림판 | 새 종이처럼 | A Light Sketchpad | Like fresh paper | 종이 테마 |
| 4 | 간단 그림판 | 무료 낙서 앱 | Simple Paint App | Free doodle app | 검색 키워드 |
| 5 | 손가락 그림판 | 배울 것 없음 | Finger Drawing Pad | Nothing to learn | 진입장벽 없음 |
| 6 | 그냥 그림판 | 복잡한 기능 없이 | Just a Drawing Pad | No clutter | 군더더기 없음 |
| 7 | 낙서용 그림판 | 심심할 때 한 장 | Doodle Pad | For quick doodles | 가벼운 용도 |
| 8 | 누구나 그림판 | 아이도 어른도 | Drawing Pad for All | Kids and adults | 사용자 범위 |

**쓰지 않을 표현**
- 전문가 느낌을 주는 말: 드로잉 툴, 스케치 워크플로우, 레이어, 벡터
- 앱 내부 용어: 캔버스, 브러시 프리셋, 해상도 (2·3장 이후에서만 사용)
- 과장 표현: 최고의, 완벽한, 올인원

### 후보군 선택 가이드
- 한 스크린샷 세트 안에서는 **하나의 후보군으로 통일**한다. (A~C를 섞지 않는다)
- 영문 제목은 Title Case, 부제목은 문장 첫 글자만 대문자로 통일한다.
- 이미지에 얹기 전 360dp 기준으로 잘림을 확인한다. 특히 `Drawing Pad for All`, `16 colors, custom`처럼 3단어인 항목은 줄바꿈 여부를 미리 점검한다.

---

## 캡처 팁

### 일반 가이드라인
- **해상도**: 최소 1080x1920
- **형식**: PNG 권장
- **비율**: 9:16 우선
- **기기**: 실제 Android 디바이스 캡처 권장
- **테마**: 밝은 테마 우선, 색상 팔레트와 종이색 캔버스가 잘 보이도록 촬영
- **상태**: 빈 화면 대신 실제 작품이 그려진 상태로 촬영하고, 광고 배너는 비행기 모드로 숨긴다

> 시드 도구: `--dart-define=DOODLE_PAD_QA_SEED_ARTWORKS=true` 로 실행하면 보관함에 프로그램 생성 작품 8점이 심어진다.
> 시드 코드 변경 후에는 `am force-stop` 뒤 재실행해야 반영된다.

### 강조할 요소
- 앱을 열고 바로 그릴 수 있는 흐름
- 10종 브러시와 16색 팔레트 + 커스텀 컬러
- Undo / Redo로 실험하기 쉬운 구조
- 갤러리 사진 위에 덧그리는 사진 드로잉
- PNG·JPEG 기기 갤러리 저장, 앱 내 작품 보관함, Android 공유
- 후원형 Premium의 3개 옵션과 광고 제거

### 스크린샷 배치 순서
1. 홈 화면과 빠른 시작
2. 풀스크린 드로잉 캔버스
3. 브러시와 색상 팔레트
4. 사진 위 드로잉
5. 작품 보관함과 저장
6. 설정 / 후원형 Premium

---

## 구글 스토어 검색어 추천

### English (primary / secondary / long-tail)
- Primary:
  - Doodle Pad
  - Drawing App
  - Sketch Pad
  - Paint App
  - Doodle Drawing

- Secondary:
  - Quick Sketch
  - Drawing Canvas
  - Save Drawing
  - Share Artwork
  - Watercolor Brush
  - Airbrush Drawing

- Long-tail:
  - simple drawing app for quick sketches
  - doodle pad with save and share
  - drawing canvas with pen marker eraser
  - sketch app with watercolor brush
  - draw on top of a photo from gallery

### 한국어 (핵심 / 보조 / 롱테일)
- 핵심:
  - 그림판
  - 드로잉 앱
  - 낙서 앱
  - 스케치 앱
  - 그림 그리기

- 보조:
  - 그림 저장
  - 그림 공유
  - 수채화 브러시
  - 에어브러시
  - 사진 위에 그리기

- 롱테일:
  - 바로 그리는 드로잉 앱
  - 저장하고 공유하는 그림판
  - 펜 마커 지우개가 있는 낙서 앱
  - 수채화 브러시가 있는 스케치 앱
  - 사진 따라 그리는 그림 앱

### 사용 팁
- 핵심 키워드는 앱 제목과 짧은 설명에 포함
- 보조 키워드는 긴 설명과 스크린샷 설명에 자연스럽게 포함
- 롱테일 키워드는 업데이트 노트와 FAQ에 활용
- `drawing`, `sketch`, `doodle`, `paint`, `canvas`, `share` 같은 상황 키워드를 함께 사용

---

## 추가 권장사항

### 타겟 사용자
- 간단히 그림을 그리고 저장하고 싶은 사용자
- 아이디어를 그림으로 빠르게 남기고 싶은 사용자
- 손가락으로 낙서하거나 색칠하고 싶은 사용자
- 사진을 따라 그리거나 사진에 메모하고 싶은 사용자
- 메신저나 SNS로 그림을 바로 공유하고 싶은 사용자

### 핵심 가치 제안
- 앱 실행 후 바로 그리기
- 10종 브러시 제공 (펜·연필·마커·붓·형광펜·만년필·크레파스·수채화·에어브러시·지우개)
- 16색 팔레트 + 커스텀 컬러와 브러시 크기 조절
- 갤러리 사진 위 드로잉과 참조 사진 보존
- 기기 갤러리 저장(PNG·JPEG), 앱 내 작품 보관함, 다시 열어 편집, 공유까지 한 흐름
- 1회 후원으로 광고 제거와 Premium 브러시 접근

### 차별화 요소
- 드로잉에 필요한 기능만 간결하게 제공
- perfect_freehand 기반으로 긋는 속도에 따라 굵기가 변하는 선
- 저장한 작품을 보관함에서 다시 열어 이어 편집 가능 (참조 사진도 함께 보존)
- 흔들어 지우기 같은 가벼운 조작
- 보상형 광고 또는 Premium으로 특수 브러시 접근 가능
- 11개 언어 지원과 라이트/다크 테마

### 경쟁 앱 대비 장점
- 튜토리얼이나 계정 없이 실행 즉시 그릴 수 있음
- 전문가용 레이어 도구 없이도 질감이 다른 브러시 10종을 제공
- 그림과 참조 사진을 함께 보관해 이어 그리기가 끊기지 않음
- 저장 포맷(PNG·JPEG)과 해상도(1x·2x·3x)를 저장 시점에 직접 선택
- 작품이 기기 로컬에만 저장되어 계정·클라우드 업로드가 필요 없음
- 구독이 아닌 1회 후원으로 광고를 영구 제거
