...
### 앱 수익 모델 (2026년 5월)
- **기본: 무료 드로잉 앱**
  - 10종 브러시 (펜 / 연필 / 마커 / 붓 / 형광펜 / 만년필 / 크레파스 / 수채화 / 에어브러시 / 지우개)
  - perfect_freehand 기반 stroke 엔진으로 자연스러운 굵기 변동
  - 16색 팔레트 + 커스텀 컬러 피커, 캔버스 배경 색상 프리셋
  - 브러시 크기 슬라이더, Undo / Redo (최대 20단계)
  - 갤러리 사진을 참조로 깔고 그 위에 그리기 (사진 위 드로잉)
  - 그린 결과를 PNG 또는 JPEG로, 1x/2x/3x 해상도를 골라 기기 갤러리에 저장
  - 그린 결과를 PNG로 캡처해 Android 공유 시트로 전달
  - 앱 내 작품 보관함: 그림을 strokes·배경색·참조 사진과 함께 저장하고 갤러리에서 다시 열어 편집
...
### D. 앱 설명 작성
- `가볍게 그리고 저장하고 다시 열어 이어 그리기`, `10종 브러시`, `16색 팔레트 + 커스텀 컬러`, `사진 위 드로잉`, `기기 갤러리 저장`, `Android 공유` 흐름을 앞부분에 배치
- 수채화 / 에어브러시는 보상형 광고 또는 Premium으로 접근 가능하다고 명시
- 무료 버전 광고 포함 안내
- Premium은 1회 후원형 구매이며 Small / Medium / Large 모두 같은 혜택임을 명확히 표기
- 구매 복원 지원 문구 포함
- 앱 내 작품 보관함에 저장한 그림은 기기 로컬에만 보관되며 클라우드 동기화는 없다는 점을 표기

### E. 리뷰 관리
- 앱 내 작품 보관함은 기기 로컬(Hive)에만 저장되며 서버로 업로드되지 않는다는 점 안내
- 공유는 Android 공유 시트를 통해 사용자가 직접 선택한 앱으로만 진행됨
- 사진 위 드로잉의 참조 사진은 작품 저장 시 앱 내부 저장소로 복사되어 보관됨
