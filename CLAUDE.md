...
- `assets`: `fonts`, `images`
- `test/`: `app/controllers/` (doodle·gallery·premium·setting·brush_type_persistence), `app/services/` (purchase·export·artwork_repository), `app/admob/` (ads_helper·ads_loading·interstitial_frequency), `app/data/brushes/` (brush_presets·airbrush_spray), `app/bindings/` (app_binding_shake_order), `app/pages/{draw,gallery,home,settings}/`, `app/mixins/`, `app/theme/`, `app/utils/`, `app/widgets/app_confirm_dialog_test.dart`, `app/helpers/fake_purchase_service.dart`, `translate_consistency_test.dart`, `ui/no_gradient_usage_test.dart`, `ui/theme_rebuild_test.dart`, `widget_test.dart`
